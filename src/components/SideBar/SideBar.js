                    ...
                    <div>
                        <div className='flex justify-between mx-20px mt-10px h-56px '>
                            ...
                            <div onClick={() => { setSideBarActive(false); setAuthorization(true) }} className='flex my-auto text-14px'>
                                <span className='mt-1 mr-1'>{localStorage.user ? "Выйти" : "Войти"}</span>
                        ...
                        <div className='ml-20px text-16px font-normal mt-2 text-blue'>
                            {arr.map(x => <Link to={`/${x.name}`}><div className='h-33px'>{x.title}</div></Link>)}
                        </div>
                        <div className='h-105px text-12px bg-input_bg font-medium'>
                            ...
                            <Link to="/"><div className='ml-20px'>Каталог</div></Link>
                        ...
                        {sidebarActive === 'Фильтры' ?
                            <div>
                                ...
                                <form onSubmit={(e) => filterJson(e)}>
                                    <div><DropList setFilterArr={setFilterArr} json={json} filters={manufacturers} title="Бренд" /></div>
                                    <div><DropList setFilterArr={setFilterArr} json={json} filters={pharmaceutical_forms} title="Категория" /></div>
